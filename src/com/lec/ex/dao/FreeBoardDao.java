...
public class FreeBoardDao {
	...
	// (1) 글목록(startRow~endRow)
		public ArrayList<FreeBoardDto> listBoard(int startRow, int endRow){
			...
			try {
				...
				while(rs.next()) {
					...
					dtos.add(new FreeBoardDto(fid, mid, mname, ftitle, fcontent,
		...
		public int writeBoard(FreeBoardDto dto) {
			...
			Connection        conn  = null;
			PreparedStatement pstmt = null;
			String sql = "INSERT INTO FREEBOARD (FID, MID, FTITLE, FCONTENT, FFILENAME, "
		...
		public void hitUp(int fid) {
			Connection        conn  = null;
			PreparedStatement pstmt = null;
			String sql = "UPDATE FREEBOARD SET FHIT = FHIT + 1 WHERE FID = ?";
			try {
				conn = ds.getConnection();
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, fid);
		...
		public FreeBoardDto getBoardNotHitUp(int fid) {
			...
			PreparedStatement pstmt = null;
			ResultSet         rs    = null;
			String sql = "SELECT F.*, MNAME FROM FREEBOARD F, MEMBER M WHERE F.MID=M.MID AND FID= ? ";
			try {
				conn = ds.getConnection();
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, fid);
				...
				if(rs.next()) {
					...
					int    findent= rs.getInt("findent");
					String fip = rs.getString("fip");
					dto = new FreeBoardDto(fid, mid, mname, ftitle, fcontent,ffileName,  
		...
		public int modifyBoard(FreeBoardDto dto) {
			int result = FAIL;
			Connection        conn  = null;
			PreparedStatement pstmt = null;
		...
		public int reply(FreeBoardDto dto) {
			int result = FAIL;
